...
export default function PlayList(props) {
  ...
  const handelGetDataTransaction = (txId, chainId) => {
    var network = chains[chainId].name;
    dispatch(handelgetAudio({ network: network, txId: txId }));
  }
  ...
  return (
    <div className="playListContiner p-md-4">
      <div className='playListHead p-md-0 p-4'>
        ...
        <h4 className='text-light m-0'>Playlist</h4>
        <button className={isLogin?"icon fs-1 btn p-0 border-0 text-success" :"icon fs-1 btn p-0 border-0 text-danger"} onClick={() => props.setShowModalWallet("wallet")}>account_balance_wallet</button>
      </div>
                      ...
                      transActionsAPI.data.map((row, index) =>
                        <tr key={index}>
                          ...
                          <td className='px-1 px-sm-2 position-sticky start-0'>
                            <div className='d-flex align-items-center '>
                              <audio id={"audio" + index} controls
                                src={blobUrl[row.transactionHash]} className='d-none'
                                onLoadedMetadata={(e) => handelSetAudioContorols(e, index)}
                                onPlay={() => handelPlay(index, "play")}
                                onPause={() => handelPlay(index, "pause")}
                              />
                              {
                                getTransActionDataAPI.Status[row.transactionHash] === "loading"
                                &&
                                <span onClick={() => handelGetDataTransaction(row.transactionHash, row.chainId)} className="audioAction loading"><i>download</i></span>
                              }
                              {
                                (getTransActionDataAPI.Status[row.transactionHash] === "idle" && audioContorols[index])
                                &&
                                <>
                                  {
                              ...
                              {
                                (getTransActionDataAPI.Status[row.transactionHash] === "rejected" || getTransActionDataAPI.Status[row.transactionHash] === undefined)
                                &&
                                <i onClick={() => handelGetDataTransaction(row.transactionHash, row.chainId)} className="audioAction notDownload">download</i>
                              }
                          ...
                          <td className='text-light px-1 px-sm-2'>{row.from === transActionsAPI.owner ? "To:" + row.to.substring(0, 4) + "..." + row.to.substring(38, 42) : "From" + row.from.substring(0, 4) + "..." + row.from.substring(38, 42)}</td>
                          {
                            (getTransActionDataAPI.Status[row.transactionHash] === "idle" && audioContorols[index])
                              ?
                              <td className='text-light px-1 px-sm-2'><a href={blobUrl[row.transactionHash]} download>{row.createDateTime.substring(0, 10)}</a></td>
                              :
